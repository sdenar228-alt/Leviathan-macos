...
**Ошибка:**
На настоящем маке клиент открывался не на весь экран: строка меню macOS и док
оставались видны (фото пользователя, 2 сентября).

**Причина:**
Два пути в коде расходились. При переключении режима из настроек
(`SetWindowParams`) DDNet на macOS превращает режим 1 («чистый fullscreen») в
desktop-fullscreen — с пометкой, что в эксклюзивном режиме игра зависает при
потере фокуса. А при **создании окна** (`IssueInit`) режим 1, который стоит по
умолчанию, по-прежнему запрашивал эксклюзивный `SDL_WINDOW_FULLSCREEN`. На
современных macOS это не даёт полноэкранного Space: окно ложится под строку
меню и док, и только поход в настройки делал экран настоящим.

**Исправление:**
`graphics_threaded.cpp`, `IssueInit`: под `CONF_PLATFORM_MACOS` (и Haiku, как в
рантаймовой ветке) режим 1 при старте тоже считается desktop-fullscreen. Windows
не затронут — правка под ifdef.

**Проверка:**
Windows: `build.bat` зелёный. macOS: сборка в CI; визуально — только на маке
пользователя после обновления, у сборочной среды экрана нет.
